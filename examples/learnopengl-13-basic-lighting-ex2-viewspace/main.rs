...
fn main() -> anyhow::Result<()> {
    ...
    let window = video
        .window("LearnOpenGL: Basic Lighting Ex - View Space", 1920, 1080)
        .opengl()
        .resizable()
        .build()?;
    ...
    let cube_light_pos = {
        ...
        rgl::uniform_matrix_4f32v(view, rgl::MatrixOrderMajor::Row, &[from_glm(&view_mat)]);
    ...
    'main: loop {
        ...
        let light_pos_viewspace =
            view_mat * glm::vec4(light_pos[0], light_pos[1], light_pos[2], 1.0);
        let light_pos_viewspace = glm::vec4_to_vec3(&light_pos_viewspace);

        {
            rgl::use_program(cube_shader_program);
            rgl::uniform_3f32(
                cube_light_pos,
                light_pos_viewspace[0],
                light_pos_viewspace[1],
                light_pos_viewspace[2],
            );
