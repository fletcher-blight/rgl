...
fn main() -> anyhow::Result<()> {
    ...
    let window = video
        .window("LearnOpenGL: Materials", 1920, 1080)
        .opengl()
        .resizable()
        .build()?;
    ...
    let (cube_light_pos, cube_light_ambient, cube_light_diffuse) = {
        ...
        rgl::uniform_3f32(material_ambient, 1.0, 0.5, 0.31);
        rgl::uniform_3f32(material_diffuse, 1.0, 0.5, 0.31);
        rgl::uniform_3f32(material_specular, 0.5, 0.5, 0.5);
        ...
        (light_pos, light_ambient, light_diffuse)
    };

    let (light_model, light_light_colour) = {
    ...
    'main: loop {
        ...
        let light_colour = [
            (total_duration_secs / 3.0 * 2.0).sin(),
            (total_duration_secs / 3.0 * 0.7).sin(),
            (total_duration_secs / 3.0 * 1.3).sin(),
        ];

        let light_colour_strength = move |strength: f32| {
            [
                light_colour[0] * strength,
                light_colour[1] * strength,
                light_colour[2] * strength,
            ]
        };

        {
            ...
            rgl::uniform_3f32v(cube_light_ambient, &[light_colour_strength(0.2)]);
            rgl::uniform_3f32v(cube_light_diffuse, &[light_colour_strength(0.5)]);
