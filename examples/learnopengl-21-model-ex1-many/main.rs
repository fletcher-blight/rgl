...
fn main() -> anyhow::Result<()> {
    ...
    let directional_light_stationary = DirectionalLight {
    ...
    directional_light_stationary
        .update(&backpack_shader_program, "directional_light_stationary")?;

    let mut directional_light_rotating = DirectionalLight {
        direction: [0.0; 3],
        colour: LightColour {
            ambient: [0.01; 3],
            diffuse: [0.8; 3],
    ...
    'main: loop {
        ...
        camera.update_position(frame_duration.as_secs_f32(), 10.0);
        ...
        directional_light_rotating.direction =
            glm::rotate_z_vec3(&glm::vec3(0.0, 1.0, 0.0), total_secs_passed / 2.0)
                .data
                .0[0];
        directional_light_rotating
            .update(&backpack_shader_program, "directional_light_rotating")?;

        for x in -5..5 {
            for y in -5..5 {
                for z in -5..5 {
                    let position_offset =
                        glm::vec3(x as f32 * 10.0, y as f32 * 10.0, z as f32 * 10.0);

                    point_light_horizontal_rotating.position =
                        (glm::rotate_y_vec3(&glm::vec3(3.0, 0.0, 0.0), total_secs_passed / 1.0)
                            + position_offset)
                            .data
                            .0[0];
                    {
                        ...
                        backpack_shader_program.set_uniform_mat4(
                            ...
                            from_glm_mat4(&glm::translate(&glm::one(), &position_offset)),
                        )?;
                        backpack_shader_program.set_uniform_mat4(
                            backpack_shader_program.find_uniform("view")?,
                            camera_view,
                        )?;
                        backpack_shader_program.set_uniform_mat4(
                    ...
                    }
                    {
                        ...
                        light_shader_program.set_uniform_mat4(
                            light_shader_program.find_uniform("view")?,
                            camera_view,
                        )?;
                        light_shader_program.set_uniform_mat4(
